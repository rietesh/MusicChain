...
class LoginScreen(Screen):
    def check_pass(self):
        ...
        Data = {"Name": name}
        r = requests.post('http://0.0.0.0:8000/ValidateUser/', data=Data)
        D = r.json()
    ...
    def create_user(self,instance):
        ...
        r = requests.post('http://0.0.0.0:8000/CreateUser/', data=Data)
...
class HomeScreen(Screen):
    def buy_music(self):
        ...
        r = requests.post('http://0.0.0.0:8000/BuyMusic/', data=Data)
        print(r.text)

    def update_labels(self):
        Data = {'acc_name': name}
        r = requests.post('http://0.0.0.0:8000/GetBalance/', data=Data)
        D = r.json()
        self.ids['balance'].text = 'Balance: '+ str(D['ans'])
        rr = requests.get('http://0.0.0.0:8000/GetLabels/')
        D = rr.json()
        D = random.sample(D,4)
...
class ProfileScreen(Screen):
    def query_account(self):
        Data = {'acc_id': name}
        r = requests.get('http://0.0.0.0:8000/QueryAccount/', data = Data)
        self.ids['response'].text = r.text

    def query_music(self,instance):
        Data = {'acc_id': name,'asset': self.cahract}
        d = requests.get('http://0.0.0.0:8000/QueryMusic/', data = Data)
        a = d.text
        re = a.replace('\n', ' ')
    ...
    def upload_music(self,instance):
        ...
        d = requests.post('http://0.0.0.0:8000/UploadMusic/', data=Data)
    ...
    def buy_coin(self,instance):
        ...
        d = requests.post('http://0.0.0.0:8000/BuyCoin/', data=Data)
        self.ids['response'].text = d.text
